<nav class="bg-gray-800" x-data="{openHidden: false}">
  <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
    <div class="relative flex h-16 items-center justify-between">
      ...
      <div class="absolute inset-y-0 left-0 flex items-center sm:hidden">
        ...
        <button x-on:click="openHidden = true" type="button" class="inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white" aria-controls="mobile-menu" aria-expanded="false">
      ...
      @auth
      <!-- Div que engloba - Campana de notificación y foto de perfil -->
        ...
        <div class="relative ml-3" x-data="{open: false}">
          ...
          <div x-show="open" x-on:click.away="open = false" class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
            ...
            <form method="POST" action="{{ route('logout') }}" x-data>
              @csrf
              <a href="{{ route('logout') }}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-2" @click.prevent="$root.submit();" {{ __('Log Out') }}>
                Cerrar sesión</a>
            </form>
          </div>
        </div>
  ...
  <!-- Mobile menu, cuando reducimos el tamaño de la ventana. -->
  <div class="sm:hidden" id="mobile-menu" x-show="openHidden" x-on:click.away="openHidden = false">
